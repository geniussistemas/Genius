...
public class QRCodeConnectionManager : IQRCodeEventNotifier
{
    ...
    public QRCodeConnectionManager(ILoggerAdapter logger, IOptions<GeniusSettings> settings, IServiceScopeFactory serviceScopeFactory)
    {
        _logger = logger;
    ...
    public async Task InitializeAndStartAsync(CancellationToken cancellationToken)
    {
        ...
        _logger.Information("Facility ID obtido: {facilityId}", facilityId);
        ...
        _gateway = new QRCodeWebSocketGateway(_logger, serverUrl, facilityId, timeToReconnect);
