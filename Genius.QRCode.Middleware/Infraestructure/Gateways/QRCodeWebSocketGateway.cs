...
namespace Genius.QRCode.Middleware.Infraestructure.Gateways;
...
public class QRCodeWebSocketGateway : IQRCodeWebSocketGateway
{
    ...
    public QRCodeWebSocketGateway(ILoggerAdapter logger, QRCodeSettings qrCodeSettings)
    {
        _client = new ClientWebSocket();
        _facilityId = qrCodeSettings.FacilityId;
        _logger = logger;
        _timeToReconnectInMs = qrCodeSettings.TimeToReconectInSeconds * 1000;
        _serverUri = new Uri($"{qrCodeSettings.WebSocketServerUrl}?facilityId={_facilityId}");
    }

    public async Task ConnectAndListenAsync(CancellationToken cancellationToken)
    {
        _logger.Information("[Facility {facilityId}] Iniciando serviço de WebSocket.", _facilityId);
